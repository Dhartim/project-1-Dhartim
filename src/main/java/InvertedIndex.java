...
public class InvertedIndex 
{
	...
	public void create(HashMap<String,Integer> termToFrequency, Object obj)
	{
		...
		for(String term:termToFrequency.keySet()) {
			Wrapper wrap = new Wrapper(obj,termToFrequency.get(term));
			if(invertedIndex.containsKey(term)==false) {
			objectsToFrequency =  new ArrayList<Wrapper>(); //list of obj
			objectsToFrequency.add(wrap);
			invertedIndex.put(term, objectsToFrequency);
		}
			else {
				objectsToFrequency = invertedIndex.get(term);
				objectsToFrequency.add(wrap);
				invertedIndex.put(term, objectsToFrequency);
			}
		}
	...
	public List<String> searchterm(String term)
	{
		...
		if(invertedIndex.containsKey(term))
		{
			...
			{
				result.add(wrap.getObj().toString());
			}
		}
		else
		{
	...
	public List<String> partialSearch(String term)
	{
		...
		for(String searchPartialTerm : invertedIndex.keySet())
		{
			...
			{
				partialResult.addAll(searchterm(searchPartialTerm)); //
				flag = 1;
			}
		}
		if(flag == 0)
...
//	{
//		ArrayList<Wrapper> display;
//		for(String term : invertedIndex.keySet())
//		{
//			System.out.println(term);
//			display = invertedIndex.get(term);
//			for(Wrapper obj : display)
//			{
//				System.out.println("Frequency:"+obj.getFrequency()+" "+obj.getObj().toString());
//			}
//		}
//	}
	//sorting passing arraylist
	/**
